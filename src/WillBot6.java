...
public class WillBot6 {
	...
	private int[] checkForPin4(int[][] board, int size, int p){//finds places to guarantee a win after next turn
		int[] point = new int[2];
		int np = 3-p;
		int[][] pin4count = new int[size][size];
		int[] win = new int[2];
		int[] pin1 = new int[2];
		int[] pin2 = new int[2];
		int[] pin3 = new int[2];
						...
						if(win != null){
							...
							if(pin1 != null){
								...
								if(pin2 != null){
									...
									pin3 = checkForPin3(newboard, newsize, p);
									if(pin3 != null){
										pin4count[x][y]++;
										newboard[pin3[0]][pin3[1]] = np;
									}else{break;}
								}
							}
						}
		...
		for(int x = 0; x < size; x++){
			for(int y = 0; y < size; y++){
				if(pin4count[x][y] > maxval){
					point[0] = x;
					point[1] = y;
					maxval = pin4count[x][y];
				}
			}
		}
		if(maxval >= 2){return point;}
